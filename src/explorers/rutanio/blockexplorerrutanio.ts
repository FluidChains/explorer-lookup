...
function parsingFunction ({ jsonResponse }: IParsingFunctionAPI): TransactionData {
  ...
  const remoteHash: string = stripHashPrefix(lastOutput.scriptPubKey, BLOCKCHAINS.rutanio.prefixes);
...
const serviceURL: ExplorerURLs = {
  main: `https://explorer.blockcore.net/ruta/api/query/transaction/${TRANSACTION_ID_PLACEHOLDER}`,
  test: `https://explorer.blockcore.net/ruta/api/query/transaction/${TRANSACTION_ID_PLACEHOLDER}`
};

export const explorerApi: ExplorerAPI = {
  serviceURL,
  serviceName: TRANSACTION_APIS.blockexplorerruta,
  parsingFunction,
  priority: -1
};
